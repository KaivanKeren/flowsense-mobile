...
/// One still open, one acknowledged with a note, one acknowledged without —
/// so the golden pins all three shapes a card can take.
List<OperatorAlert> _alerts() => [
      ...
      OperatorAlert(
        ...
        name: 'Simpang Tujuh',
        level: CongestionLevel.macet,
        raisedAt: _t0.subtract(const Duration(minutes: 80)),
        acknowledgedBy: 'Ismail',
        acknowledgedAt: _t0.subtract(const Duration(minutes: 60)),
      ...
      OperatorAlert(
        ...
        name: 'Simpang Bae',
        level: CongestionLevel.macet,
        raisedAt: _t0.subtract(const Duration(hours: 23, minutes: 32)),
        acknowledgedBy: 'Rina',
        acknowledgedAt: _t0.subtract(const Duration(hours: 23)),
      ),
    ];
...
void main() {
  ...
  testWidgets('PeringatanScreen at 360x800', (tester) async {
    ...
    await expectLater(
      find.byType(PeringatanScreen),
      matchesGoldenFile('peringatan_screen.png'),
    );
  });
}
